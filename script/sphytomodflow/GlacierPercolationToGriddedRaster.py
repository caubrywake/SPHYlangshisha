# -*- coding: utf-8 -*-
"""
Created on Thu Apr  6 19:51:45 2023
...
for attr_col in glacperc.columns:
    if attr_col != 'geometry' and attr_col != 'DN':
        ...
        output_file = f'D:/UU/glacperc/v4/GlacpM{num_days_str[:3]}.{num_days_str[3:]}'  # Add dot between front three and last three digits
...
for attr_col in glacperc.columns:
    if attr_col != 'geometry' and attr_col != 'DN':
        # Calculate the number of days between the minimum date and the current attribute column date
        num_days = (datetime.strptime(attr_col, date_format) - min_date).days
        ...
        print(f'Rasterized raster for attribute column {attr_col} saved to: {output_file}')
'''
        # Plot raster
        fig, ax = plt.subplots(1, figsize=(10, 10))
        im = ax.imshow(rasterized, extent=grid_extent, cmap='gray', origin='upper')
        ax.invert_yaxis()
...
# Import Suvbzone percolation raster

# Import Glacier percolation rater

# add them together

# regulaer transfor to shapefile
